...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.initialize_weights()  # me: changed

    def initialize_weights(self):
        # initialization
        # initialize (and freeze) pos_embed by sin-cos embedding
        # me rem
        # pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), cls_token=True) # me: rem
        pos_embed = sincos_pos_embed(
            self.pos_embed.shape[-1], self.patch_embed.num_patches, cls_token=True
        )
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        # decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], int(self.patch_embed.num_patches**.5), cls_token=True) # me: rem
        decoder_pos_embed = sincos_pos_embed(
...
# # mae_vit_base_patch16_dec512d8b 110M
...
def build_mae_vit_encoder_from_checkpoint(
    checkpoint_path: str,
    device: torch.device,
    cfg,
