...
def add_labeled_traces_and_decorations(fig: go.Figure, g: pd.DataFrame, glen: int = 20):
    ...
    fig.add_trace(
        go.Scatter(
            x=indices, y=data[:, 0], mode="lines+markers",
            name="IMU X", line=dict(width=1.5, color="red"),
    ...
    fig.add_trace(
        go.Scatter(
            x=indices, y=data[:, 1], mode="lines+markers",
            name="IMU Y", line=dict(width=1.5, color="blue"),
    ...
    fig.add_trace(
        go.Scatter(
            x=indices, y=data[:, 2], mode="lines+markers",
            name="IMU Z", line=dict(width=1.5, color="green"),
    ...
    fig.add_trace(
        go.Scatter(
            x=[x0, indices[-1]], y=[0, 0], mode="lines",
    ...
    fig.add_trace(
        go.Scatter(
            x=lbl_x, y=[y_max - 0.5] * len(lbl_x),
            mode="text", text=lbl_text, textposition="top center",
    ...
    fig.update_yaxes(
        range=[y_min, y_max],
        tickmode="array", tickvals=[y_min, 0, y_max],
        row=1, col=2
    )
